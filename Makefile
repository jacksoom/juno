...
LD_FLAGS = -X github.com/jacksoom/juno/v4/cmd.Version=$(VERSION) \
	-X github.com/jacksoom/juno/v4/cmd.Commit=$(COMMIT)
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -name '*.pb.go' -not -path "./venv" | xargs goimports -w -local github.com/jacksoom/juno
.PHONY: format
